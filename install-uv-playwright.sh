...
create_or_use_venv() {
  if [[ -d "$VENV_DIR" ]]; then
    ...
    log "Creating virtual environment '$VENV_DIR' with uv..."
    uv venv "$VENV_DIR"
  fi
  # shellcheck disable=SC1091
  source "$VENV_DIR/bin/activate"
  log "Activated venv: $VENV_DIR (python: $(python -V 2>&1))"
}
...
install_py_deps() {
  ...
  log "Installing/upgrading dotenv..."
...
main() {
  ...
  log "Done!"
  echo
  echo "Next time, activate your environment with:"
